...
function renderCommitInfo(data, commits) {
   ...
   const busiestDay = [...dayCounts.entries()].sort((a, b) => b[1] - a[1])[0][0];

   dl.append('dt').text('Most Active Day');
   dl.append('dd').text(busiestDay);

   const maxDepth = d3.max(data, (d) => d.depth);
   dl.append('dt').text('Max File Depth');
   dl.append('dd').text(maxDepth);

   const maxLines = d3.max(data, (d) => d.line)
   dl.append('dt').text('Max file line length');
...
let xScale, yScale; // declare at top level
...
function onTimeSliderChange() {
   ...
   updateScatterPlot(data, filteredCommits); // <-- pass both data and filteredCommits
